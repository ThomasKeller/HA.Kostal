...
public class KostalParserTests
{
    ...
    public void parser_should_parse_page_when_piko_is_off()
    {
        ...
        Assert.That((long)measurement.Values.First(v => v.Name == "DownloadTime_ms").Value, Is.EqualTo(123));
        Assert.That((double)measurement.Values.First(v => v.Name == "DailyEnergy_kWh").Value, Is.EqualTo(3.70));
        Assert.That((int)measurement.Values.First(v => v.Name == "ProducedEnergy_kWh").Value, Is.EqualTo(75217));
    ...
    public void parser_should_parse_page_when_piko_is_on()
    {
        ...
        Assert.That((long)measurement.Values.First(v => v.Name == "DownloadTime_ms").Value, Is.EqualTo(123));
        Assert.That((double)measurement.Values.First(v => v.Name == "DailyEnergy_kWh").Value, Is.EqualTo(3.81));
        Assert.That((int)measurement.Values.First(v => v.Name == "ProducedEnergy_kWh").Value, Is.EqualTo(75221));
        Assert.That((int)measurement.Values.First(v => v.Name == "CurrentACPower_W").Value, Is.EqualTo(1130));

        Assert.That((int)measurement.Values.First(v => v.Name == "L1.Power_W").Value, Is.EqualTo(373));
        Assert.That((int)measurement.Values.First(v => v.Name == "L1.Voltage_V").Value, Is.EqualTo(229));
        Assert.That((int)measurement.Values.First(v => v.Name == "L2.Power_W").Value, Is.EqualTo(378));
        Assert.That((int)measurement.Values.First(v => v.Name == "L2.Voltage_V").Value, Is.EqualTo(228));
        Assert.That((int)measurement.Values.First(v => v.Name == "L3.Power_W").Value, Is.EqualTo(379));
        Assert.That((int)measurement.Values.First(v => v.Name == "L3.Voltage_V").Value, Is.EqualTo(225));

        Assert.That((double)measurement.Values.First(v => v.Name == "String1.Current_A").Value, Is.EqualTo(0.62));
        Assert.That((int)measurement.Values.First(v => v.Name == "String1.Voltage_V").Value, Is.EqualTo(524));
        Assert.That((double)measurement.Values.First(v => v.Name == "String2.Current_A").Value, Is.EqualTo(1.38));
        Assert.That((int)measurement.Values.First(v => v.Name == "String2.Voltage_V").Value, Is.EqualTo(613));
